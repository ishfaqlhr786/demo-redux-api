...
function UseJsonMatTable() {
  ...
  const handleRowUpdate = (newData, oldData, resolve) => {
    ...
    if(errorList.length < 1){
      axios.patch("http://localhost:7777/posts/"+ oldData.id)
      .then(res => {
  ...
  const handleRowAdd = (newData, resolve) => {
    ...
    if(errorList.length < 1){ //no error
      axios.post("http://localhost:7777/posts", newData)
      .then(res => {
        let dataToAdd = [...data];
  ...
  const handleRowDelete1 = (oldData, resolve) => {
    ...
    axios.delete("http://localhost:7777/posts/"+oldData.id)
...
export default UseJsonMatTable;
